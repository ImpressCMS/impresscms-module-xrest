...
case "dashboard":
    ...
    $indexAdmin->addInfoBoxLine(_XREST_AM_XREST_SUMANDTOTAL, "<label>"._XREST_AM_XREST_TOTAL_ACTIVE_PLUGINS."</label>", $plugins_handler->getCount(new icms_db_criteria_Item('`active`', '1','=')), ($plugins_handler->getCount(new icms_db_criteria_Item('`active`', '1','='))>0?'Green':'Red'));
    $indexAdmin->addInfoBoxLine(_XREST_AM_XREST_SUMANDTOTAL, "<label>"._XREST_AM_XREST_TOTAL_INACTIVE_PLUGINS."</label>", $plugins_handler->getCount(new icms_db_criteria_Item('`active`', '0','=')), ($plugins_handler->getCount(new icms_db_criteria_Item('`active`', '0','='))>0?'Green':'Red'));
    $indexAdmin->addInfoBoxLine(_XREST_AM_XREST_SUMANDTOTAL, "<label>"._XREST_AM_XREST_TOTAL_FAILED_PLUGINS."</label>", intval(IcmsCache::read('xrest_plugins_failed')), (intval(IcmsCache::read('xrest_plugins_failed'))>0?'Red':'Green'));
	$indexAdmin->addInfoBoxLine(_XREST_AM_XREST_SUMANDTOTAL, "<label>"._XREST_AM_XREST_TOTAL_TABLES."</label>", $tables_handler->getCount(new icms_db_criteria_Item('`view`', '0','=')), ($tables_handler->getCount(new icms_db_criteria_Item('`view`', '0','='))>0?'Green':'Red'));
	$indexAdmin->addInfoBoxLine(_XREST_AM_XREST_SUMANDTOTAL, "<label>"._XREST_AM_XREST_TOTAL_VIEWS."</label>", $tables_handler->getCount(new icms_db_criteria_Item('`view`', '1','=')), ($tables_handler->getCount(new icms_db_criteria_Item('`view`', '1','='))>0?'Green':'Red'));
	...
	$indexAdmin->addInfoBoxLine(_XREST_AM_XREST_SUMANDTOTAL, "<label>"._XREST_AM_XREST_AVERAGE_FIELDS."</label>", number_format(($fields_handler->getCount()+1)/($tables_handler->getCount(new icms_db_criteria_Item('`view`', '0','='))+1),2), (number_format(($fields_handler->getCount()+1)/($tables_handler->getCount(new icms_db_criteria_Item('`view`', '0','='))+1),2)>0?'Green':'Red'));
	$lastplugin = IcmsCache::read('xrest_plugins_last');
	...
	$lastcleanup = IcmsCache::read('xrest_cleanup_last');
...
<!-- End Form Validation JavaScript //-->');
	...
	for($key=0;$key<=4;$key++) {
		switch ($key) {
			case 2:
				$donationform[$key] =  sprintf($donationform[$key], $GLOBALS['xoopsConfig']['sitename'] . ' - ' . (strlen(icms::$user->getVar('name'))>0?icms::$user->getVar('name'). ' ['.icms::$user->getVar('uname').']':icms::$user->getVar('uname')), icms::$user->getVar('email'), XOOPS_LICENSE_KEY, strtoupper($GLOBALS['xrestModule']->getVar('dirname')),  strtoupper($GLOBALS['xrestModule']->getVar('dirname')). ' '.$GLOBALS['xrestModule']->getVar('name'));
				break;
		}
	}
