...
class IcmsFolderHandler
{
    ...
    {
        if (empty($path)) {
            $path = ICMS_TRUST_PATH . '/caches/icms_cache';
        }
        if ($mode) {
    ...
    function IcmsFolderHandler($path, $create = false, $mode = false)
    {
    ...
    {
        if (IcmsFolderHandler::isWindowsPath($path)) {
            return '\\';
        }
        return '/';
    }
    ...
    {
        if (IcmsFolderHandler::isSlashTerm($path)) {
            return $path;
        }
        return $path . IcmsFolderHandler::correctSlashFor($path);
    }
    ...
    /**
     * Returns true if the File is in a given IcmsPath.
     *
     * @return bool
     * @access public
     */
    function inIcmsPath($path = '')
    {
        $dir = substr($this->slashTerm(ICMS_ROOT_PATH), 0, - 1);
        $newdir = $dir . $path;
