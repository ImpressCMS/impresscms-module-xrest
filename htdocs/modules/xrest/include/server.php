...
if (isset($_REQUEST['xrestplugin'])) {
	...
	if (is_object($plugin)) {
		require_once((ICMS_ROOT_PATH.'modules/xrest/plugins/'.$plugin->getVar('plugin_file')));
		if ((!$result = IcmsCache::read('xrest_results_'.$_REQUEST['xrestplugin'].'_'.md5(implode(':',$values))))&&$plugin->getVar('active')==true) {
			$result = array();
			...
			if (!empty($opfunc)) {
				...
				IcmsCache::write('xrest_results_'.$_REQUEST['xrestplugin'].'_'.md5(implode(':',$values)), $result, $GLOBALS['xrestModuleConfig']['cache_seconds']);
			}
		}	
	}
}
...
IcmsCache::write('xrest_plugins_last', $GLOBALS['xrestPlugin'], $GLOBALS['xrestModuleConfig']['run_cleanup']*2);
exit(0);
?>