...
function checkright($function_file, $username, $password){
	...
	$online_handler =& icms::handler('icms_online');		

	$GLOBALS['xrestModule'] = $module_handler->getByDirname('xrest');
	$GLOBALS['xrestPlugin']['user'] = $username;

	$plugin_handler = icms_getModuleHandler('plugins', 'xrest');
	...
	if ($uid <> 0){
		...
		icms::$user = &$member_handler->getUser( $uid );
		$_SESSION['xoopsUserGroups'] = icms::$user->getGroups();
...
function check_for_lock($function_file, $username, $password) {
	...
	$retn = false;
	if ($result = IcmsCache::read('lock_'.$function_file.'_'.$username)) {
		...
		IcmsCache::delete('lock_'.$function_file.'_'.$username);
		IcmsCache::write('lock_'.$function_file.'_'.$username, $result, $GLOBALS['xrestModuleConfig']['lock_seconds']+mt_rand(1, $GLOBALS['xrestModuleConfig']['lock_random_seed']));
		return $retn;
	}
}
...
function mark_for_lock($function_file, $username, $password) {
	...
	if ($result = IcmsCache::read('lock_'.$function_file.'_'.$username)) {
		...
		IcmsCache::delete('lock_'.$function_file.'_'.$username);
		IcmsCache::write('lock_'.$function_file.'_'.$username, $userip, $GLOBALS['xrestModuleConfig']['lock_seconds']+mt_rand(1, $GLOBALS['xrestModuleConfig']['lock_random_seed']));
		return array('ErrNum'=> 9, "ErrDesc" => _XC_MI_NOPERMFORPLUGIN);
	}
}
?>